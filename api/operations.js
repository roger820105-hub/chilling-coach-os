...
async function googleShiftSync(req,res){
 ...
 if(leavePayload.length){const refs=leavePayload.map(x=>encodeURIComponent(x.external_ref)),existing=await rows(url,headers,`leave_requests?external_source=eq.google_sheets&external_ref=in.(${refs.join(",")})&select=id,external_ref`),byRef=Object.fromEntries(existing.map(x=>[x.external_ref,x.id])),fresh=[];for(const item of leavePayload){const id=byRef[item.external_ref];if(id)await rows(url,headers,`leave_requests?id=eq.${id}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify(item)});else fresh.push(item)}if(fresh.length)await rows(url,headers,"leave_requests",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify(fresh)})}
