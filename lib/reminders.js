...
module.exports=async function runReminders(req,res){
 ...
 const [users,roles,links,students,packages,sessions]=await Promise.all([
  rows(c.url,c.headers,"users?is_active=eq.true&line_user_id=not.is.null&select=id,line_user_id,display_name"),rows(c.url,c.headers,"user_roles?select=user_id,role"),rows(c.url,c.headers,"coach_students?ended_at=is.null&select=coach_id,student_id"),rows(c.url,c.headers,"students?status=eq.active&select=id,name"),rows(c.url,c.headers,"packages?status=eq.active&select=student_id,remaining_sessions,expires_at"),rows(c.url,c.headers,`sessions?status=eq.scheduled&scheduled_at=gte.${now.toISOString()}&scheduled_at=lte.${new Date(now.getTime()+25*3600000).toISOString()}&select=student_id,coach_id,scheduled_at`)
 ]);
 const names=Object.fromEntries(students.map(x=>[x.id,x.name])),coachByStudent=Object.fromEntries(links.map(x=>[x.student_id,x.coach_id])),groups=Object.fromEntries(users.map(x=>[x.id,{user:x,classes:[],renewals:[]}])) ;
